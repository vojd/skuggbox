...
impl Painter {
    ...
    pub fn set_texture(&mut self, tex_id: egui::TextureId, delta: &egui::epaint::ImageDelta) {
        ...
        // TODO(mathias): Creating the texture upfront - borrow checker errors in "or_insert_with" when passing in self
        let texture = unsafe { self.gl.create_texture().unwrap() };
        let glow_texture = *self.textures.entry(tex_id).or_insert_with(|| texture);
        unsafe {
